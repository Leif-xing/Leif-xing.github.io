...
async function save_url(URL, random_key) {
    let is_exist = await LINKS.get(random_key)
    console.log(is_exist)
    if (is_exist == null)
        return await LINKS.put(random_key, URL), random_key
    else
        console.log("random key already exists!")
}
...
async function handleRequest(request) {
    ...
    if (request.method === "POST") {
        ...
        if (config.unique_link) {
            stat, random_key = await save_url(req["url"], random_key)
        } else {
            stat, random_key = await save_url(req["url"], random_key)
        }
        console.log(stat)
    ...
    if (!path) {
        ...
        const html = await fetch("https://leif-xing.github.io/url_shorten/index.html")
